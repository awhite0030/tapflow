...
| `swipe` | `{ from, to, durationMs? }` | 두 지점 사이를 스와이프합니다. 좌표는 0~1. `durationMs`는 밀리초 단위 정수이며 기본값 300, 최대 2147483647입니다. |
...
`timeout`은 초 단위이며 기본값은 10초, 최대 2147483.647초(약 24.8일)입니다. 셀렉터마다 개별로 지정할 수 있습니다.
