...
네 가지를 따로 말합니다. **설치돼 있는가**, **승인됐는가**, **켜져 있는가**, 그리고 **지금 도는 확장이 이 tapflow가 싣고 온 것과 같은 버전인가**. 뒤의 두 항목이 따로 있는 이유는 앞의 것들이 다 맞는데도 동작하지 않는 상태가 있기 때문입니다 — 교체 직후 재시작 전이 하나이고, 필터가 꺼져 있는 경우가 다른 하나입니다. 꺼져 있는 상태는 자기 버전이 없습니다. 확장은 활성으로 남으므로 버전은 전부 맞게 읽히는데 아무것도 필터링되지 않습니다.

버전이 어긋났다고 나오면 어느 쪽이 뒤처졌는지에 따라 할 일이 다릅니다. 이 tapflow가 싣고 온 것이 더 새롭다고 하면 `tapflow migrate net-filter`가 설치합니다. 맥이 돌리는 것이 더 새롭다고 하면 대신 이 체크아웃을 최신으로 올리세요. 그 방향은 migrate가 거부합니다. 새 필터를 덮어쓰면 그것에 기대는 에이전트가 망가지기 때문입니다. 재시작하라는 안내가 나오면 맥을 재시작하세요.
...
`tapflow migrate net-filter`는 무언가를 교체하기 전에 필터를 먼저 끄므로 이런 일이 일어나지 않습니다. 다만 어떤 tapflow 릴리즈보다도 오래된 손빌드 필터가 올라가 있는 맥에서는 여전히 일어날 수 있습니다. 그 빌드는 끄라는 요청을 이해하지 못할 수 있기 때문입니다. 이 단계를 건너뛴 경우 명령이 그렇게 알려줍니다.
