...
iOS 시뮬레이터를 오프라인으로 만들려면 에이전트 맥에 tapflow 네트워크 확장이 설치돼 있어야 합니다.

**브라우저에서 보는 사람은 고칠 수 없습니다.** 아래는 전부 에이전트가 도는 맥 앞에서, 관리자 권한으로 해야 합니다.

확장이 없는 상태에서도 버튼은 눌리고 앱은 오프라인 화면을 그립니다. 하지만 요청은 실제로 성공하므로 그 상태로 확인한 오프라인 동작은 신뢰할 수 없습니다.
...
승인 창은 에이전트가 도는 맥에만 뜹니다. 다른 자리에서 브라우저로 접속한 사람에게는 아무것도 보이지 않고, 그래서 대시보드에도 재시도 버튼이 없습니다.
...
## iOS: 오프라인으로 바꿨는데 앱이 계속 온라인이라고 합니다 {#network-hooks-failed}

확장은 정상인데(위 절에서 확인) 앱만 오프라인을 모르는 경우입니다. 네트워크 확장과는 다른 고장입니다. 트래픽은 실제로 끊겨 있습니다. 앱에게 끊겼다고 알리는 쪽이 실패했습니다.

**기기를 재시작하세요.** 이 통지 장치는 기기가 부팅할 때 설치되므로 재부팅으로 대부분 복구됩니다.

재부팅해도 매번 같으면 시뮬레이터 런타임이 호환되지 않는 경우입니다. 새 Xcode나 새 시뮬레이터 런타임으로 옮긴 직후라면 [이슈로 알려주세요](https://github.com/jo-duchan/tapflow/issues). 그동안 오프라인 확인은 앱이 그리는 배너가 아니라 네트워크 요청이 실패하는지로 하세요.

::: tip 하이브리드 앱의 WebView는 원래 통지를 못 받습니다
WebView 안에서 도는 화면은 오프라인 배너를 그리지 않습니다. 통지 장치가 WebView 프로세스에는 올라가지 않기 때문입니다. 고장이 아니라 알려진 제약입니다. WebView의 네트워크 요청 자체는 정상적으로 실패합니다.
:::
