...
## iOS: 오프라인인데 앱이 계속 온라인이라고 합니다 {#network-hooks-failed}

트래픽은 끊겨 있는데 앱만 그걸 모르는 상태입니다.

**시뮬레이터를 재부팅하세요.** 부팅할 때 다시 설치되므로 대부분 이걸로 복구됩니다.
...
WebView 안에서 도는 화면은 오프라인 배너를 그리지 않습니다. 고장이 아니라 알려진 제약입니다. WebView의 네트워크 요청 자체는 정상적으로 실패합니다.
:::
