...
## 네트워크 버튼이 "This Mac can't take devices offline"이라고 합니다 {#network-not-set-up}

iOS 시뮬레이터를 네트워크에서 떼려면 맥에 시스템 확장이 한 번 설치돼 있어야 하고, 그게 아직 안 된
상태입니다. **버그가 아니라 상태입니다** — 그리고 브라우저에서 보는 사람은 고칠 수 없습니다.
에이전트가 도는 맥 앞의 사람이 해야 합니다.

Android는 해당 없습니다. 비행기 모드라 준비가 필요 없습니다.

설치·승인·제거는 [기기를 네트워크에서 떼기](/ko/guide/network-control)에 있습니다.
