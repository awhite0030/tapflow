...
이미 설치된 확장을 교체하면 맥을 재시작해야 완료됩니다. **재시작 전까지는 이전 버전이 계속 동작합니다** — 파일은 새 것인데 커널에서 도는 것은 옛 것인 상태라, 대시보드는 여전히 준비되지 않았다고 말합니다.
...
버전이 어긋났다고 나오면 `tapflow migrate net-filter`를 다시 돌리거나, 재시작하라는 안내가 나오면 맥을 재시작하세요.
