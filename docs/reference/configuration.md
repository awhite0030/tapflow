...
| `TAPFLOW_AGENT_GRACE_MS` | — | `15000` (15 s) | How long a session stays alive after its agent's connection drops, waiting for that agent to come back. An agent registers about a second after its process starts, so the default covers a restart several times over; the open tab says it is waiting rather than showing a frame that has stopped updating. Until the window closes the device is not offered to anyone else. Set to a small value to make the session end promptly instead. |
