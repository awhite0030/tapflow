...
- `src/` — `DeviceAgent` interface, `AgentRegistry`, shared types, `createLogger` (leveled console logger), and the **optional capability interfaces** an agent implements alongside `DeviceAgent` rather than inside it: `AudioStreamCapability`, `NetworkControlCapability`. That split is the ISP rule two lines above — a platform that cannot do one of these implements neither the interface nor a stub, and advertises the capability string instead.
