...
- **Keyboard and screen-reader focus survives when a row leaves a list that stays a list** ([#833](https://github.com/jo-duchan/tapflow/issues/833)). When a status filter is active and a build row's status is changed to one the filter excludes, the row leaves the list. Focus now correctly moves to the search box (the next logical fallback) instead of dropping to the top of the page.
