...
- **Foldables can be folded from the browser** ([#797](https://github.com/jo-duchan/tapflow/issues/797)). A device whose screens rearrange now reports the postures it offers, and the toolbar shows a control that moves between them — Folded, Rear display, Half open, Unfolded on a Pixel 9 Pro Fold. The device frame follows the screen when it changes, because the agent re-sends it rather than describing the screen it had at boot. Posture names and labels come from the platform's own agent, so a device that folds differently needs no change in the relay or the dashboard.
